...
export async function runSeed(prisma: any) {
  ...
  const seedRoleMap = new Map<string, string[]>([
    [admin.id, ["ADMINISTRATOR", "REPORT_READER"]],
    [participant.id, ["PARTICIPANT"]],
    [reviewer.id, ["REVIEWER"]],
    [appealHandler.id, ["APPEAL_HANDLER"]],
  ]);
  ...
  await createSeedPendingManualReview(prisma, {
    now,
...
async function createSeedModuleBundle(prisma: any, adminId: string, now: Date, input: SeedModuleInput) {
  ...
  return {
    ...
    promptTemplateVersionId: prompt.id,
    mcqSetVersionId: mcqSet.id,
    mcqQuestionIds: mcqSet.questions.map((question: { id: string }) => question.id),
  };
}

async function createSeedPendingManualReview(
  prisma: any,
  input: {
    now: Date;
