...
function buildAssessment(overrides: Partial<LlmStructuredAssessment> = {}): LlmStructuredAssessment {
  return {
    ...
    criterion_rationales: {
      relevance_for_case: "The submission addresses the task clearly.",
      quality_and_utility: "The content is useful and actionable.",
      iteration_and_improvement: "Iteration is documented clearly.",
      human_quality_assurance: "Manual QA is explained clearly.",
      responsible_use: "Responsible-use considerations are present.",
    },
...
describe("Local integration assessment policy suite", () => {
    ...
    const result = await createAssessedSubmission(
      ...
      [
        buildAssessment({
          ...
          criterion_rationales: {
            relevance_for_case: "Submission does not address the task requirements.",
            quality_and_utility: "Content is not substantive enough for useful evaluation.",
            iteration_and_improvement: "No iteration evidence is provided.",
            human_quality_assurance: "No QA evidence is provided.",
            responsible_use: "No responsible-use detail can be assessed.",
          },
          improvement_advice: [
            "Provide explicit MCQ answers and a structured practical reflection.",
            "Include iteration notes showing refinement and QA steps.",
          ],
          red_flags: [
            {
  ...
  it.each([
    ...
    ["missing_mcq_and_notes", "high"],
    ["missing_iteration_and_qa", "high"],
    ["r001", "high"],
      ...
      const result = await createAssessedSubmission(
        ...
        [
          buildAssessment({
            ...
            pass_fail_practical: false,
            criterion_rationales: {
              relevance_for_case: "Submission does not address the required task content.",
              quality_and_utility: "The response is not substantive enough to evaluate utility.",
              iteration_and_improvement: "No iteration evidence is present.",
              human_quality_assurance: "No QA evidence is present.",
              responsible_use: "No responsible-use detail can be assessed.",
            },
            improvement_advice: [
              "Provide a complete MCQ response.",
              "Include a substantive practical reflection with explicit iteration notes.",
              "Document any quality assurance steps performed on the submission.",
            ],
            red_flags: [
              {
                code: rawRedFlagCode,
  ...
  it("TC-POL-YELLOW-001 keeps high-risk safety cases under review", async () => {
    ...
    const result = await createAssessedSubmission(
      ...
      [
        buildAssessment({
          rubric_scores: {
            relevance_for_case: 1,
            quality_and_utility: 1,
            iteration_and_improvement: 1,
            human_quality_assurance: 0,
            responsible_use: 0,
          },
          rubric_total: 3,
